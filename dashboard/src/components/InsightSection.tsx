...
const InsightSection: React.FC<InsightSectionProps> = ({ insights, title = "AI Strategy Insights" }) => {
    return (
        <div className="bg-slate-900 rounded-3xl p-4 sm:p-8 text-white relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 relative z-10">
                {insights.map((insight, idx) => (
                    <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-6 flex flex-col justify-between border-l-4 border-l-red-500">
                        <div>
                            <h3 className="font-bold text-base sm:text-lg mb-2 text-white">{insight.topic}</h3>
                            <p className="text-slate-300 text-xs sm:text-sm mb-4 leading-relaxed">{insight.analysis}</p>
                        </div>
